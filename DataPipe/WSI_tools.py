...
class Load_a_WSI_with_valid_regions(MapTransform):
    """
    ...
    def __call__(self, sample: Dict) -> Dict:
        ...
        """
        # STEP 1: Open WSI
        logging.info(f"Load_a_WSI_with_valid_regions: read {sample[self.image_key]}")
        WSI_image_obj: OpenSlide = self.reader.read(sample[self.image_key])

        # STEP 2: Select the valid regions on the WSI, in one bbox
        logging.info("Load_a_WSI_with_valid_regions: get bbox")
        ...
        logging.info(f"Load_a_WSI_with_valid_regions: level0_bbox: {level0_bbox}")
