...
from PuzzleAI.ModelBase.ROI_models.attention_modules import simam_module, cbam_module, se_module
...
class Hybrid_feature_map_Embed(nn.Module):  # HybridEmbed from timm
    ...
    def forward(self, x):
        ...
        proj(x).shape:  batch, slide_embed_dim, patch_height_num, patch_width_num
        flatten(2).shape:  batch, slide_embed_dim, patch_num
...
class Last_feature_map_Embed(nn.Module):
    ...
    def __init__(self, patch_size=1, feature_size=(7, 7), feature_dim=2048, embed_dim=768,
        ...
        self.grid_size = (feature_size[0] // self.patch_size[0], feature_size[1] // self.patch_size[1])  # slide_feature

        self.num_patches = self.grid_size[0] * self.grid_size[1]

        # use the conv to split the slide_feature by the following design:
        self.proj = nn.Conv2d(in_channels=feature_dim, out_channels=embed_dim,
...
class Focus_Embed(nn.Module):  # Attention guided module for hybridzing the early stages CNN feature
    ...
    def __init__(self, patch_size=1, target_feature_size=(7, 7), feature_size=(56, 56), feature_dim=256, embed_dim=768,
        ...
        super().__init__()
        patch_size = to_2tuple(patch_size)
        feature_size = to_2tuple(feature_size)  # slide_feature size of the current feature map

        target_feature_size = to_2tuple(target_feature_size)  # slide_feature size of the last feature map
        ...
        # cheak target_feature map can be patchlize to slide_feature
...
class Focus_Aggressive(nn.Module):  # Attention guided module for hybridzing the early stages CNN feature
    ...
    def __init__(self, patch_size=1, target_feature_size=(7, 7), feature_size=(56, 56), feature_dim=256, embed_dim=768,
                 Attention_module=None, norm_layer=nn.LayerNorm):
        super().__init__()
        patch_size = to_2tuple(patch_size)  # slide_feature size of the last feature map
        feature_size = to_2tuple(feature_size)
...
class VisionTransformer(nn.Module):  # From timm to review the ViT and ViT_resn5
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        Args:
            ...
            patch_size (int, tuple): slide_feature size
            ...
            embed_layer (nn.Module): slide_feature embedding layer
...
class Stage_wise_hybrid_Transformer(nn.Module):
    ...
    then, last feature map slide_feature embedding is used to connect the CNN output to the decoder1 input
    ...
    def __init__(self, backbone, num_classes=1000, patch_size=1, embed_dim=768, depth=4, num_heads=8, mlp_ratio=4.,
        ...
        '''
        self.cls_token_1 = nn.Parameter(torch.zeros(1, 1, slide_embed_dim))
        self.pos_embed_1 = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, slide_embed_dim))

        self.cls_token_2 = nn.Parameter(torch.zeros(1, 1, slide_embed_dim))
        self.pos_embed_2 = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, slide_embed_dim))

        self.cls_token_3 = nn.Parameter(torch.zeros(1, 1, slide_embed_dim))
        self.pos_embed_3 = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, slide_embed_dim))

        self.cls_token_4 = nn.Parameter(torch.zeros(1, 1, slide_embed_dim))
        self.pos_embed_4 = nn.Parameter(torch.zeros(1, num_patches + self.num_tokens, slide_embed_dim))
        '''
    ...
    def forward_features(self, x):
        ...
        if self.cls_token_num != 0:  # concat cls token
            ...
            x = torch.cat((cls_token_0, x), dim=1)  # 增加classification head slide_feature
