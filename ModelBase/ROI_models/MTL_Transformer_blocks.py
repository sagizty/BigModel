"""
Transformer blocks   Script  ver： May 4th 11:30
...
class Transformer_blocks(nn.Module):
    def __init__(self, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4., qkv_bias=True, drop_rate=0.,
...
# this two class(PatchEmbed and VisionTransformer) just for you to know about the ViT, we don't use them
...
if __name__ == '__main__':
    ...
    model.load_state_dict(ViT_backbone_weights, False)
    ...
    x = torch.randn(2, 234, 768).to(dev)
    y = model(x)
    print(y.shape)

    x = torch.randn(2, 345, 768).to(dev)
    y = model(x)
    print(y.shape)
