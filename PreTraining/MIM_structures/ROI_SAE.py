"""
SAE Model for PuzzleTuning   Script  ver： Oct 24th 19:00
...
from ModelBase.ROI_models.VPT_structure import VPT_ViT
...
class ShuffledAutoEncoderViT(VPT_ViT):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3,
        ...
        if decoder is not None:
            ...
            self.decoder_pred = nn.Linear(decoder_rep_dim, patch_size ** 2 * in_chans, bias=True)  # decoder to slide_feature
            ...
            # Decoder use a FC to reconstruct image, unlike the Encoder which use a CNN to split slide_feature
            self.decoder_pred = nn.Linear(decoder_embed_dim, patch_size ** 2 * in_chans, bias=True)  # decoder to slide_feature
    ...
    def patchify_decoder(self, imgs, patch_size=None):
        ...
        # use reshape to split slide_feature [B, CLS, H, W] -> [B, CLS, h_p, patch_size, w_p, patch_size]
    ...
    def unpatchify(self, x, patch_size=None):
        """
        Decoding encoded slide_feature tokens
    ...
    def fix_position_shuffling(self, x, fix_position_ratio, puzzle_patch_size):
        ...
        # set puzzle slide_feature
        # ids_?.unsqueeze(-1).repeat(1, 1, D)
        # [B,?_patches] -> [B,?_patches,1] (at each place with the idx of ori slide_feature) -> [B,?_patches,D]
        ...
        # batch&slide_feature-wise shuffle is needed else the restore will restore all puzzles
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        # use mask as a slide_feature indicator [B, num_patches, D] -> [B, num_patches]
        ...
        # MSE loss
        loss = (pred - target) ** 2
        loss = loss.mean(dim=-1)  # [B, num_patches], mean loss on each slide_feature pixel
...
if __name__ == '__main__':
    ...
    '''
    if not os.path.exists('./temp-figs/'):
        os.mkdir('./temp-figs/')

    image_tensor_path = '../../Utils/temp-tensors/color.pt'
